...
from pages.qingjia_peixunjia_page import qingjia_peixunjia_page


class Test_qingjia_peixunjia(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.peixunjia = qingjia_peixunjia_page(self.driver)
    ...
    撤销申请
    '''
    def test_peixunjia_zhengchang_success(self):
        text_qingjia = self.peixunjia.peixunjia_zhengchang_success()
    ...
    '''培训假，申请8.5天，申请失败，'''
    def test_peixunjia_9day_fail(self):

        time.sleep(1)
        fail_text = self.peixunjia.peixunjia_9day_fail()
        self.assertEqual(fail_text, '公司培训假最多申请8天', msg='断言失败，产检假申请超出8天的申请时间时，没有给出请假余额已用完提示')
        time.sleep(1)
        # self.cxqj.shenpi_chexiaoshenqing()